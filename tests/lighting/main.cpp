...
Geode *createCube() {
	// vertex array
	osg::ref_ptr<Vec3Array> vertexArray = new Vec3Array();
	...
	// face array
	osg::ref_ptr<DrawElementsUInt> faceArray = new DrawElementsUInt(PrimitiveSet::TRIANGLES, 0);
	...
	osg::ref_ptr<Geometry> geometry = new Geometry();
	geometry->setVertexArray(vertexArray.get());

	geometry->setNormalArray(normalArray.get(), osg::Array::BIND_PER_VERTEX);
	geometry->addPrimitiveSet(faceArray.get());

	osg::ref_ptr<Geode> cube = new Geode();
