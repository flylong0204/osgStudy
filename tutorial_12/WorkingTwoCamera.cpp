...
void createView(osgViewer::CompositeViewer *viewer, osg::ref_ptr<osg::Group> root, osg::ref_ptr<osg::GraphicsContext> gc,
	osgGA::TrackballManipulator *manipulator, int x, int y, int width, int height)
{
	double left;
	double right;
	double top;
...
int main( int argc, char **argv )
{
   ...
   createView(&viewer, rootNode, gc, mainManipulator, 0, 0, width, height / 2);

   createView(&viewer, rootNode, gc, tankManipulator, 0, height / 2, width / 2, height / 2);

   createView(&viewer, rootNode, gc, thirdManipulator, width / 2, height / 2, width / 2, height / 2);
